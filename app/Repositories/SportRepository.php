...
class SportRepository
{
    ...
    {
        return Sport::with($with)->where($where)->first();
    }
    ...
    {
        return Sport::with($with)->where($where)->get();
    }
    ...
    public static function loadList($request)
    {
        try {
            ...
            $list = Sport::orderBy($sortBy, $sortOrder)->where('status', '!=', 'deleted');
    ...
    {
        try {
            $model = Sport::where(['id' => $request->id])->first();
            if (! empty($model)) {
