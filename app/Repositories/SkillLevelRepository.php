...
class SkillLevelRepository
{
    ...
    {
        return SkillLevel::with($with)->where($where)->first();
    }
    ...
    {
        return SkillLevel::with($with)->where($where)->get();
    }
    ...
    public static function loadList($request)
    {
        try {
            ...
            $list = SkillLevel::where('status', '!=', 'deleted')->orderBy($sortBy, $sortOrder);
    ...
    {
        try {
            $model = SkillLevel::where(['id' => $request->id])->first();
            if (! empty($model)) {
