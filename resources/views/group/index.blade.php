    ...
    <div class="page-title-row d-sm-flex align-items-center justify-content-between">
        ...
        @if($userData->user_type == 'admin')
            <div class="right-side mt-2 mt-md-0">
                <a href="{{route('user.addGroupForm', ['user_type'=>$userType])}}" class="btn btn-secondary ripple-effect-dark text-white">
                    Add Group
                </a>
            </div>
        @endif
...
<script>
    ...
    function loadList(url) {
        ...
        url = url || "{{route('user.loadGroupList', ['user_type' => $userType])}}";
    ...
    function changeStatus(id, status) {
        ...
        bootbox.confirm('Are you sure you want to ' + statusType + ' this group ?', function(result) {
            if (result) {
                var url = "{{route('common.changeGroupStatus', ['id'=>'%recordId%'])}}";
