    ...
    <div class="page-title-row d-sm-flex align-items-center justify-content-between">
        ...
        @if($userType == 'admin')
            <div class="right-side mt-2 mt-md-0">
                <a href="{{route('user.addSkillLevelForm', ['user_type'=>$userType])}}" class="btn btn-secondary ripple-effect-dark text-white">
                    Add
                </a>
            </div>
        @endif
...
<script>
    ...
    function loadList(url) {
        ...
        url = url || "{{route('common.loadSkillLevelList')}}";
    ...
    function changeStatus(id, status) {
        ...
        bootbox.confirm('Are you sure you want to ' + statusType + ' this skill level ?', function(result) {
            if (result) {
                var url = "{{route('common.changeSkillLevelStatus', ['id'=>'%recordId%'])}}";
