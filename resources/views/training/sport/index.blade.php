    ...
    <div class="page-title-row d-sm-flex align-items-center justify-content-between">
        ...
        @if($userType == 'admin')
            <div class="right-side mt-2 mt-md-0">
                <a href="{{route('user.training.sport.addForm', ['user_type'=>$userType])}}" class="btn btn-secondary ripple-effect-dark text-white">
                    Add
                </a>
            </div>
        @endif
...
<script>
    ...
    function loadList(url) {
        ...
        url = url || "{{route('common.training.sport.list')}}";
    ...
    function changeStatus(id, status) {
        ...
        bootbox.confirm('Are you sure you want to ' + statusType + ' this sport ?', function(result) {
            if (result) {
                var url = "{{route('common.training.sport.changeStatus', ['id'=>'%recordId%'])}}";
