@extends('layouts.app')
@section('head')
    <title> Rewards Management | Add</title>
@endsection
    ...
    <div class="content-wrapper">
        ...
        <section class="content white-bg">
            <form id="addForm" class="form-head" method="POST" novalidate autocomplete="false"
                action="{{ route('common.addRewardManagement') }}">
                @csrf
                <div class="form-group permission-checkbox">
                    <label class="form-check form-switch max-fit">
                        <input class="form-check-input" onchange="enableGamification(this)" type="checkbox" role="switch" name="is_gamification" id="gamification">
                        <div class="checkbox__checkmark"></div>
                        <span class="form-check-label fw-bold">Gamification</span>
                    </label>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        <div class="row" id="required-inputs">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="hidden" name="feature" id="featureField">
                                    <select class="form-control form-select" onChange="selectFeature()" id="featureKeyField"
                                        name="feature_key">
                                        <option value="">Select Feature</option>
                                        <option value="use-recipe">Use a Recipe</option>
                                        <option value="rate-recipe">Rate a Recipe</option>
                                        <option value="watch-exercise-video">Watch an exercise video</option>
                                        <option value="watch-training-video">Watch an training video</option>
                                        <option value="watch-workout-video">Watch a workout video</option>
                                        <option value="rate-video">Rate a Video</option>
                                        <option value="log-health-markers">Log your Health Markers</option>
                                        <option value="log-health-measurement">Log your Health Measurement</option>
                                        <option value="log-water-intake">Log your Water Intake</option>
                                        <option value="upgrade-your-subscription">Upgrade your Subscription</option>
                                        <option value="build-own-workout">Build your own workout</option>
                                        <option value="complete-workout">Complete a workout</option>
                                        <option value="create-workout-goal">Create a Workout Goal</option>
                                        <option value="achieve-workout-goal">Achieve your Workout goal</option>
                                        <option value="participate-group-training-session">Participate in a Group Training
                                            Session</option>
                                    </select>
                ...
                <div class="btn_row text-center">
                    <button type="button" class="btn btn-secondary ripple-effect-dark btn-120" id="addBtn"
                        onClick="addRewardManagement()">Add<span id="addBtnLoader" class="spinner-border spinner-border-sm"
...
@section('js')
    <!-- {!! JsValidator::formRequest('App\Http\Requests\RewardManagementRequest', '#addForm') !!} -->

    <script>
        const gameList = @json($games);
        ...
        function addRewardManagement() {
            ...
            if ($('#addForm').valid()) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('common.addRewardManagement') }}",
                    data: formData,
                    ...
                    error: function(err) {
                        ...
                        if (err.status === 422) {
                            ...
                            _toast.error(errors.message || 'Rewards points management not created.');
                        }
                    },
                });
            }
