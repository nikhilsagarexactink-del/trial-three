...
<div class="content-wrapper">
    <div class="page-title-row d-sm-flex align-items-center justify-content-between">
        <div class="left-side">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Recurring Broadcasts</li>
                </ol>
            </nav>
            <!-- Breadcrumb End -->
            <!-- Page Title Start -->
        ...
        <div class="right-side mt-2 mt-md-0">
            <a href="{{route('user.recurringBroadcastAddForm', ['user_type'=>$userType])}}" class="btn btn-secondary ripple-effect-dark text-white">
                Add
            </a>
        </div>
    </div>
...
<script>
     ...
     * @response object.
     */
    function loadRecurringBroadcastList(url) {
        $("#listId").html('{{ajaxTableListLoader()}}');
        url = url || "{{route('common.loadRecurringBroadcastList')}}";
    ...
    function changeStatus(id, status) {
        ...
        bootbox.confirm('Are you sure you want to ' + statusType + ' this broadcast ?', function(result) {
            if (result) {
                var url = "{{route('common.changeRecurringBroadcastStatus', ['id'=>'%recordId%'])}}";
    ...
    $('.sorting').on('click', function(e) {
        ...
        loadRecurringBroadcastList(false);
    });
