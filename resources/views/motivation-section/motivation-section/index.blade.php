@extends('layouts.app')
<title>Motivation Section </title>
@section('content')
        ...
        <div class="page-title-row d-sm-flex align-items-center justify-content-between">
            <div class="left-side">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Motivation Section</li>
                    </ol>
                </nav>
                <!-- Breadcrumb End -->
                <!-- Page Title Start -->
            ...
            @if ($userType == 'admin')
                <div class="right-side mt-2 mt-md-0">
                    <a href="{{ route('user.motivationSection.addForm', ['user_type' => $userType]) }}"
                ...
                <div class="form-group permission-checkbox">
                    <label class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" onChange="updateSettings()"
                            {{ !empty($settings) && $settings['motivation-section-status'] == 'enabled' ? 'checked' : '' }}
                            role="switch" name="motivation-section-status" id="getting_started_status">
...
@section('js')
    <script>
        ...
        function loadList(url) {
            ...
            url = url || "{{ route('common.motivation.loadList') }}";
        ...
        function changeStatus(id, status) {
            ...
            bootbox.confirm('Are you sure you want to ' + statusType + ' this motivation section video ?', function(
                result) {
                if (result) {
                    var url = "{{ route('common.motivation.changeStatus', ['id' => '%recordId%']) }}";
        ...
        function updateSettings() {
            ...
            $.ajax({
                ...
                data: {
                    'motivation-section-status': status ? 'enabled' : 'disabled'
                },
                success: function(response) {
                    $('#updateBtn').prop('disabled', false);
                    $('#updateBtnLoader').hide();
                    if (response.success) {
                        _toast.success(status ? 'Motivation section successfully enabled.' :
                            'Motivation section successfully disabled.');
                    } else {
