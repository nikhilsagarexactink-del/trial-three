    ...
    <div class="content-wrapper">
        <div class="page-title-row d-sm-flex align-items-center justify-content-between">
            <div class="left-side">
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Step Counter</li>
                    </ol>
                </nav>
                <!-- Breadcrumb End -->
                <!-- Page Title Start -->
            ...
            <div class="right-side mt-2 mt-md-0">
                <a href="{{ route('user.stepCounter.addStepForm', ['user_type' => $userType]) }}"
                    class="btn btn-secondary btn-orange ripple-effect-dark text-white">
                    INPUT RESULT
                </a>
                <a class="ms-2 btn btn-primary" href="{{ route('user.stepCounter.setGoal', ['user_type' => $userType]) }}">
                    RESET MY GOAL
                </a>
            </div>
        </div>
        <!-- Fitness Challenge Widget -->
            <x-challenge-alert type="step-counter"/>
        <!--Header Text start-->
        <div>
...
@section('js')
    <script>
                ...
                onClick: function(evt, data) {
                    ...
                    if (chartData && chartData.length) {
                        ...
                        bootbox.confirm('Are you sure you want to edit this date detail ?', function(result) {
                            if (result) {
                                var url =
                                    "{{ route('user.stepCounter.editStepForm', ['user_type' => $userType]) }}";
        ...
        function getUserGoalLog() {
            $.ajax({
                type: "GET",
                url: "{{ route('common.stepCounter.getGoalLog') }}",
                data: {},
        ...
        function calculateDonutChartPercent(chart, data) {
            ...
            if (data.chartOne) {
                ...
                let chartOneData = [data.chartOne.totalStepValues, totalGoal];
            ...
            if (data.chartTwo) {
                ...
                totalGoal = (totalGoal > 0) ? totalGoal : 1;
                let chartTwoData = [data.chartTwo.totalStepValues, totalGoal];
            ...
            if (data.chartThree) {
                ...
                totalGoal = (totalGoal > 0) ? totalGoal : 1;
                let chartThreeData = [data.chartThree.totalStepValues, totalGoal];
        ...
        function calculateLogs(chart = "chartTwo") {
            if (chart == "chartOne") {
                let chartOneData = [];
                let chartOneDates = [];
                userGoalLogs.chartOne.dates.forEach((date) => {
                    chartOneDates.push(moment(date).format('MM-DD-YYYY'));
                    let dayValue = userGoalLogs.chartOne.data.find((obj) => obj.date == date);
                    if (dayValue) {
                        chartOneData.push(dayValue.step_value);
                    } else {
                        chartOneData.push(0);
                    }
                    //barChartData.push(date);
                })
                barChart.data.labels = chartOneDates;
            ...
            if (chart == "chartTwo") {
                ...
                if (dayValue) {
                    chartTwoData.push(dayValue.step_value);
                } else {
                    chartTwoData.push(0);
                }
                    //barChartData.push(date);
            ...
            if (chart == "chartThree") {
                ...
                userGoalLogs.chartThree.dates.forEach((date) => {
                    ...
                    if (dayValue) {
                        chartThreeData.push(dayValue.step_value);
                    } else {
